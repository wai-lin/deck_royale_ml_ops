...
def ask_deck_advice(user_input: str, player_id: str = None):
    ...
    with open(f"{PROMPTS_FOLDER}/developer_prompt.md", "r") as f:
        instructions_dev = f.read()

    popular_decks = decks.get_popular_decks()
    instructions_dev = instructions_dev.replace("{{current_meta_decks}}", decks_to_md(popular_decks))
